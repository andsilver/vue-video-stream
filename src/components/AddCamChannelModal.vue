<template>
  <div class="">
    <!-- Modal Component -->
    <b-modal ref="modalAddCamChannel" 
             id="modal-add-cam-channel"
      ...
      <template slot="modal-header">
        <div style="width: 100%">
          <b-row>
            ...
            <b-col><span>New IPCam</span></b-col>
          </b-row>
        </div>
      </template>
      <div>
        ...
        <div v-if="error && !error.role" 
          ...
          <div v-if="error.subscription"
            ...
            <router-link to="/subscribe?category=ipcam&action=upgrade">
        ...
        <div v-if="error">
          <div v-if="error.role"
            ...
            <p style="min-height:80px;">
              You do not have an active IPCam Streaming subscription. Would you like to activate our 30 days trial ?</p>
            <br>
            <div v-if="trialError" 
            ...
            <div class="text-right">
              ...
              <div v-else>
                ...
                <div style="font-size:13px;margin:12px;">
                  <router-link to="/subscribe?category=ipcam&action=upgrade">Get more options</router-link>
                </div>
              </div>
            </div>
            <br>
...
export default {
  name: "AddCamChannelModal",
  async mounted() {
    this.$refs.modalAddCamChannel.$on("hide", this.onDismiss);
    this.$refs.modalAddCamChannel.$on("shown", this.onInit);
    this.regions = await StreamService.getAvailableRegions('ipcam');
  },
  data() {
    return {
      processing: false,
  ...
  methods: {
    async onInit() {
      ...
      const hasCamStreamSub = !!_.find(sub.addonSubscriptions, { category: 'ipcam' })
      if (!hasCamStreamSub) {
    ...
    async forceActivateTrial () {
      ...
      try {
        await SubscriptionService.requestTrial('ipcam')
      } catch(e) {
        this.trialError = e
        return
      } finally {
      ...
      // on trial activated
      this.$notify({ group: 'success', text: 'IPCam Trial activated successfully' })

      // update intercom config
      window.Intercom('update', { 
        ipcamstreamTrial: true,
        ipcamTrialExpiry: new Date(Date.now()+(30*24*3600*1000))
      });
    ...
    async onSaveChannel() {
      ...
      try {
        const stream = await StreamService.addCamStream(
          this.channel.name,
          this.channel.region
        );
        ...
        // update intercom config
        window.Intercom('update', { "ipcamCreated": true });
    ...
    dismiss() {
      this.$refs.modalAddCamChannel.hide();
      this.onDismiss();
    },
    onDismiss() {
      this.resetForm();
