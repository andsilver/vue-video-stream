<template>
    <div class="container view-wrapper dvr-widget-wrapper" style="padding:0">
      <div v-if="processing">
        <div class="page-placeholder" style="height:400px;position:absolute;">
      ...
      <div v-else class="dvr-widget placeholder">loading media ..</div>
      <iframe width="650" 
              height="0" 
              :src="dvrIframeUrl"
              class="dvr-widget"
              allowfullscreen
              border="0"
              :style="{ opacity: processing?0.3:1 }"></iframe>
      <br>
      <p class="text-muted text-right"
         style="font-size:13px;"
         :style="{ opacity: processing?0:1 }">Note: this recorded content is only for demonstration purpose.</p>
      <div v-show="!processing" style="margin-top:30px;">
        <p>Do you want to record your streaming content?</p>
        <button class="modal-button highlight"
                onclick="Intercom('show')">Contact Us Now</button>
      </div>
  </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    // compose demo dvr embed url
    this.dvrIframeUrl = `https://dvr.castr.io/live_d1e97b40ebf311e8aa8915cc9f48337a/embed.html?dvr=true&region=fr`
  },
  data () {
    return {
      processing: true,
